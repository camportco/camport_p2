...
ajaxpageclass.connect=function(pageurl, divId){
	...
	page_request.open('GET', pageurl+bustcacheparameter+"&divId="+divId, true)
...
ajaxpageclass.createBook.prototype={
	...
	buildpagination:function(selectedpage){ //build pagination links based on length of this.pageinfo.pages[]
		...
		if (this.pageinfo.pages.length<=1){ //no 0 or just 1 page
			...
			paginateHTML+='<li><a href="#previous" rel="'+(selectedpage-1)+'">&lsaquo;</a></li>\n' //previous link HTML
			...
			paginateHTML+='<li><a href="#next" rel="'+(selectedpage+1)+'">next &rsaquo;</a></li>\n' //next link HTML
			paginateHTML+='</ul></div>'
		}// end construction
